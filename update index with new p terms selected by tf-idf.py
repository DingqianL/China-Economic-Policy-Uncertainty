# -*- coding: utf-8 -*-
"""
Created on Tue Jan 19 10:58:04 2021
...
# for RMRB
path1 = r'C:\Users\dingq\Documents\200_academic\220_Academic_My dissertation\225_data_txt\RMRB_compelete\2020'
...
np1 = rexepu.keywords(flat_articles, rexepu.new_pp11)
np2 = rexepu.keywords(flat_articles, rexepu.new_pp12)
np3 = rexepu.keywords(flat_articles, rexepu.new_pp3)
np4 = rexepu.keywords(flat_articles, rexepu.new_pp6)
np5 = rexepu.keywords(flat_articles, rexepu.new_pp7)
np6 = rexepu.keywords(flat_articles, rexepu.new_pp9)
np7 = rexepu.keywords(flat_articles, rexepu.new_pp10)
np8 = rexepu.keywords(flat_articles, rexepu.new_pp5)
...
if len(e)==len(u)==len(n_epu)==len(np)==len(flat_dates):
    print('Pass')
else:
    print('Alert')


if len(e)==len(p)==len(u)==len(epu)==len(mpu)==len(mepu)==len(tpu)==len(tepu)==len(flat_articles)==len(flat_dates):
    print('Pass')
else:
    print('Alert')
...
epu_test=articlelist.filter(['date','n_epu'],axis=1)
epu_test.head(10)
...
epudailyrmrb = epudailyrmrb.reset_index()
epudailyrmrb['year'] = epudailyrmrb['date'].dt.year
epudailyrmrb['month'] = epudailyrmrb['date'].dt.month


epudailyrmrb1 = epudailyrmrb.groupby(['year', 'month']).sum()

###### for GMRB
path2 = r'C:\Users\dingq\Documents\200_academic\220_Academic_My dissertation\225_data_txt\gmrb\2020'
...
os.chdir(path)
epudaily_update.to_csv(r'update202012_join.csv')
...
epu_update.to_csv(r'update2020_on_D_epu12.csv')
...
epu_update.to_csv(r'update2020_on_D_tpu12.csv')
## for monthly
...
epu_update_M.to_csv(r'update2019_on_M_epu202012_check.csv')
tpu_update_M.to_csv(r'update2019_on_M_tpu202012_check.csv')
